...
class DoclingProcessorConverterDeployment:
    ...
    def init_metrics(self):
        if not self._metrics_initialized:
            ...
            # this doesn't work, enabling timings is added into convert/manager
            ...
            timings_hist_buckets = [
                ...
                # Coarser tail buckets so a genuine aberration (stuck OCR/layout
                # call, etc.) gets resolution instead of collapsing into +Inf.
    ...
    def emit_metrics(self, metrics: list, tenant_id: str):
        ...
        def _observe_all(hist: Histogram, values: list) -> None:
            ...
            hist.set_default_tags({"tenant_id": tenant_id, "replica_tag": replica_tag})
            for value in values:
                hist.observe(value)
    ...
    def emit_metrics_for_document(self, metrics: list, tenant_id: str) -> None:
        """Emit metrics for a document assembled outside process_converter_request,
        e.g. a slice-finalized document merged on the coordinator actor, which has
        no metrics/histogram objects of its own."""
        if not self.config.generate_metrics:
            return
        self.init_metrics()
    ...
    async def process_converter_request(
        ...
        if isinstance(request, PassthroughTaskRequest):
            ...
            if isinstance(conv_results, ConverterFailureResult):
                if self.config.generate_metrics:
                    self.emit_failure_metrics(tenant_id=tenant_id)
                return conv_results
            ...
            try:
                ...
                if effective_pages > self.config.max_page_slice_size:
                    ...
                    try:
                        ...
                        del slice_refs
                    # The coordinator actor has no metrics/histogram objects of
                    # its own; forward to a converter replica to observe them.
                    if self.config.generate_metrics:
                        tenant_id = task.metadata.get("tenant_id", "default")
                        await self.converter_handle.emit_metrics_for_document.remote(
